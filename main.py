...
#
while True:
    print('Ход осуществляется с помощью команд троек [X,Y,Action],\n'
          'где X и Y — координаты клетки на поле, а Action – действие, которое необходимо совершить:'
          '\nFlag — установить флажок на соответвующую клетку, пометив её как предположительно содержащую бомбу;'
          '\nOpen – раскрыть содержимое клетки')
    start_game()
